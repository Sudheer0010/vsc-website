...
export function MarketLetterPreview({
  ...
  return (
    <motion.article
      className={`group relative bg-[#FFFFFF] border border-rule rounded-xl p-8 hover:border-accent-gold/40 transition-all duration-300 flex flex-col justify-between select-none ${className}`}
      {...animProps}
    >
      <div>
        ...
        {/* Serif Headline */}
        <h3 className="font-display text-2xl sm:text-3xl text-ink font-normal leading-[1.25] tracking-tight mb-4 group-hover:text-accent-gold transition-colors duration-200">
        ...
        <p className="font-mono text-xs sm:text-sm text-ink-soft leading-relaxed mb-6">
          {excerpt}
        </p>
      </div>
      ...
      <div className="pt-4 border-t border-rule flex items-center justify-between">
        <span className="font-mono text-[10px] tracking-[0.2em] text-ink-muted group-hover:text-accent-gold transition-colors uppercase font-medium">
          READ BRIEFING &rarr;
        </span>
