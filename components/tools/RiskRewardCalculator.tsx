...
export function RiskRewardCalculator() {
  ...
  const { stopInvalid, targetInvalid, qtyInvalid } = useMemo(
    () => validateRiskReward(entry, stop, target, quantity),
    [entry, stop, target, quantity],
  );

  const result = useMemo<RiskRewardResult>(
    () => computeRiskReward(entry, stop, target, quantity),
    [entry, stop, target, quantity],
  );
