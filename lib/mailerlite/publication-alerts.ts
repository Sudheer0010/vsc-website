...
import { fetchActiveGroupSubscribers, importSubscribersToGroup, type MailerLiteSubscriber } from "./subscribers";
 ...
 * subscriber list down to those not yet current, then (if any) import them
 * — as one bulk call, not one request per subscriber — into that
 * publication's alert trigger group with its fields. Note and Letter
 * fields never mix — each call only ever builds the field set for its
 * own `type`.
 */
async function runOne(
  ...
  const outcome = await importSubscribersToGroup(
    alertGroupId,
    eligible.map((s) => ({ email: s.email, fields })),
    apiToken
  );
  ...
  if (outcome.timedOut) {
    return {
      ...
      triggered: 0,
      failed: 0,
      skipped: "import job did not reach a terminal state within the poll window",
    };
  }
  ...
  return {
    ...
    latestId: latest.id,
    eligible: eligible.length,
    triggered: outcome.imported + outcome.updated,
 ...
 * who aren't confirmed imported (errored, or the job timed out before we
 * could confirm) are simply reported as not triggered and stay eligible
 * for the next run, rather than the log claiming a success that the import
 * job never actually confirmed.
 */
