...
/**
 * Research — Reading Desk (Design Lab prototype).
 *
 * Same Cinematic Signal / Luminous Editorial language as the redesigned
 * Home/About/Offerings (ContourField seed 71, Reveal springs, StepRule,
 * font-editorial italic accent) instead of the production page's
 * cream-and-card-grid system.
 *
 * The brief for this page is different from Offerings: Research is not a
 * decision funnel, it is the index for VSC's actual publishing record — so
 * the redesign leans on real dates, real counts and real reading times
 * (all read from the same data/ modules the production page reads from)
 * rather than a persuasive narrative arc. Nothing below is invented copy;
 * every number, title and link comes from data/ or lib/.
 *
 * IA change from production: "Risk Tools" and "Reading Desk" — two
 * identical bordered cards on the current page — are merged into one
 * two-drawer "Shelf" panel (#shelf, with #tools and #reading-desk anchors
 * inside it) so the utility layer reads as one connected apparatus instead
 * of two unrelated tiles. Everything else keeps its section identity:
 * Letters and Trading Insights stay under "Time-bound — what we observed",
 * Frameworks and the Shelf stay under "Timeless — what we believe" (the
 * same grouping the production page's own comments describe).
 */
 ...
 * that exists to orient, not decorate (per the brief: "motion used for
 * orientation, not decoration"). A reader scrolling through a long index
 * should always see which chapter they're in without hunting for a heading.
 */
...
export function ResearchRedesign() {
  ...
  return (
    <main className="relative w-full bg-canvas">
      {/* ================================================================
          ...
          Replaces the production hero's headline + statistics-card pair:
          ...
          production stats demoted to a footer line rather than the
          visual centerpiece.
         ================================================================ */}
      ...
      {/* ================================================================
          ...
          via getReadingTime — same helpers the production page calls.
         ================================================================ */}
      ...
      {/* ================================================================
          5. FRAMEWORK LIBRARY — dark dossier chapter, same register as
          About's "Lessons" break and the homepage's Five Gates. A numbered
          pipeline list, not a flat dark rectangle: each row carries its
          question, its description and its version.
         ================================================================ */}
      <section
        id="framework-library"
        className="relative w-full scroll-mt-24 overflow-hidden border-b border-vsc-dark-hairline bg-[#0E1A14] py-24 sm:py-32"
      >
        <ContourField
          seed={34}
          layers={2}
        ...
        <div className="relative mx-auto max-w-[1200px] px-6 sm:px-10">
          <Reveal className="max-w-[64ch]">
            ...
            <p className="mt-5 max-w-[58ch] text-[16px] leading-relaxed text-vsc-dark-ink-muted">
              Five questions. One decision process. Each answer feeds the next — versioned, dated, changes
              documented.
            </p>
          </Reveal>
              ...
              {frameworkLibrary.map((fw, i) => {
                ...
                return (
                  <Link
                    ...
                    className="group -mx-3 flex flex-col gap-3 rounded-lg px-3 py-6 transition-colors duration-200 hover:bg-vsc-dark-ink/[0.04] sm:-mx-4 sm:flex-row sm:items-center sm:gap-6 sm:px-4"
                  >
                    <span className="shrink-0 self-start font-mono text-[13px] text-vsc-dark-accent sm:self-center">
                      {String(i + 1).padStart(2, "0")}
                      <span className="text-vsc-dark-ink-muted"> / {String(frameworkLibrary.length).padStart(2, "0")}</span>
                    </span>
                    <div className="flex-1">
                      <div className="flex flex-wrap items-baseline gap-x-3 gap-y-1">
                        <span className="font-display text-[19px] font-semibold text-vsc-dark-ink transition-colors duration-200 group-hover:text-white sm:text-[21px]">
                          {fw.title}
                        </span>
                        {version && (
                          <span className="font-mono text-[11px] text-vsc-dark-ink-muted">v{version.version}</span>
                        )}
                      </div>
                      <p className="mt-1.5 text-[14.5px] leading-snug text-vsc-dark-ink-muted transition-colors duration-200 group-hover:text-vsc-dark-ink/75">
                        {fw.question} — {fw.desc}
                      </p>
                    </div>
                    <span
                      aria-hidden="true"
                      className="shrink-0 self-start font-mono text-sm text-vsc-dark-accent opacity-0 transition-all duration-200 group-hover:translate-x-0.5 group-hover:opacity-100 sm:self-center"
                    >
                      &rarr;
                    </span>
                  </Link>
      ...
      {/* ================================================================
          ...
          internal rule) instead of the production page's two identical,
