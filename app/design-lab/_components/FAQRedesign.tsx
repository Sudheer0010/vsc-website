...
/**
 * FAQ — Knowledge Desk (Design Lab prototype).
 *
 * Unlike Offerings/Research, this is deliberately NOT a Cinematic Signal
 * hero page — the brief calls FAQ a utility page first. It borrows the same
 * tokens (ink/growth-green, font-display/mono/editorial, Reveal) but skips
 * ContourField, dark sections and scroll-triggered drama entirely: the
 * whole point is fast scanning, not a cinematic arrival.
 *
 * IA change from production: the six categories no longer live behind a
 * tab switcher with a duplicated sidebar sub-list (see FAQAccordion.tsx —
 * the active category's questions were rendered twice, once as scrollspy
 * links in the sidebar and once as the accordion itself). Here every
 * category is one continuous, anchorable section; a sticky pill rail
 * scrollspies across them instead of gating content behind a click. All
 * copy is read from data/faq.ts — nothing here is invented.
 */
          ...
          {hasComparison && (
            <div className="mt-5 grid grid-cols-1 gap-3 sm:grid-cols-2">
              <div className="rounded-vsc-md border border-growth/20 bg-growth-wash p-4">
                <h4 className="font-mono text-[11px] font-semibold uppercase tracking-wider text-growth-deep">
                  Trading business
                </h4>
              ...
              <div className="rounded-vsc-md border border-rose-500/15 bg-canvas-sunk p-4">
                <h4 className="font-mono text-[11px] font-semibold uppercase tracking-wider text-rose-500/80">
                  Gambling
                </h4>
                <ul className="mt-2.5 flex flex-col gap-1.5 text-[13px] text-ink-muted">
...
export function FAQRedesign() {
  ...
  // Deep-link: /design-lab/faq#q-risk-1 opens and scrolls to that exact question.
  useEffect(() => {
